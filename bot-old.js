...
async function createCombinedImage(product, options = {}) {
  ...
  const [prodBuf] = await Promise.all([
    // Fetch product image with timeout and fallback (skip 403 immediately)
    (async () => {
      ...
      try {
        const res = await fetchFunc(gambar, { timeout: 6000 });
        if (res && res.ok) {
          ...
          console.log(`⚠️ Image fetch blocked (${res.status}), skipping image`);
          return null;
        } else {
    ...
    (async () => {
      ...
      try {
        ...
        if (res && res.ok) {
          ...
          console.log(`⚠️ Bulk: Image blocked (${res.status}), skipping`);
          return null;
        }
      } catch (e) {
